...
def generate_mock_option_chain(spot_price, expiry_date=EXPIRY_DATE):
...
def get_option_chain(expiry_date=EXPIRY_DATE):
    ...
    if TEST_MODE:
        ...
        return generate_mock_option_chain(mock_market.current_spot, expiry_date)
...
def execute_entry(signal, current_time):
    ...
    # Calculate quantity (using slippage-adjusted price)
    quantity = FIRST_TRADE_CAPITAL / entry_price_with_slippage
    ...
    print(f"Capital Used: ₹{FIRST_TRADE_CAPITAL}")
...
def execute_reversal(original_trade_id, current_price, current_time):
    """
    ...
    reversal_price = current_price * 0.95  # Assume slight discount
    
    quantity = REVERSAL_CAPITAL / reversal_price
    reversal_trade = TradePosition(
        trade_id=len(state.active_trades) + 1,
        option_type=reversal_type,
        strike=original_trade.strike,
    ...
    print(f"Type: BUY {reversal_type} @ Strike ₹{original_trade.strike}")
    print(f"Entry Price: ₹{reversal_price:.2f}")
    print(f"Quantity: {quantity:.2f}")
    print(f"Capital Used: ₹{REVERSAL_CAPITAL}")
...
def update_positions(chain, spot, current_time):
    ...
    for trade_id, exit_price, reason in trades_to_close:
        ...
        if reason == "SL_HIT" and len(state.closed_trades) == 1:
            ...
            execute_reversal(trade_id, exit_price, current_time)
            ...
            try:
                ...
                if FORCE_ENTRY and not state.trading_started:
                    ...
                    if opt:
                        ...
                        trade_id = execute_entry(forced_signal, now)
                        if trade_id:
                ...
                if signal and not state.trading_started:
                    trade_id = execute_entry(signal, now)
